...
# TopologyZoo
metrics <- loadRData("../../data/r-files/hyperflow/pathDelay_HF_5C.Rda")

# LinkRemover
metrics_LR <- loadRData("../../data/r-files/hyperflow/pathDelay_HF_5C_LR.Rda") 
...
lm <- lm(RTT ~ Path.Delay, data = train)


pred <- predict(lm, test)
### see http://r-statistics.co/Linear-Regression.html
actuals_preds <- data.frame(cbind(actuals=test$RTT, predicteds=pred))
