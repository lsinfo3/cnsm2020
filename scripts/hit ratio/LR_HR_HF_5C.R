...
# TopologyZoo
metrics <- loadRData("../../data/r-files/hyperflow/metrics_HF_5C.Rda")

# LinkRemover
metrics_LR <- loadRData("../../data/r-files/hyperflow/metrics_HF_5C_LR.Rda") 
...
lm <- lm(Hitratio ~ Betweenness, data = train) # HyperFlow


pred <- predict(lm, test)
### see http://r-statistics.co/Linear-Regression.html
actuals_preds <- data.frame(cbind(actuals=test$Hitratio, predicteds=pred))
